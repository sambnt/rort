...
module Rort.Examples.Model where
...
import Data.Bits ((.|.))
import Rort.Util.Wavefront (loadObjUnique)
...
main = do
  ...
  withWindow width height "Example: Model" $ \win -> do
    ...
    runResourceT $ do
      let
        cfg = VkSettings { requiredExtensions =
                         ...
                         , applicationInfo =
                             Vk.ApplicationInfo
                               (Just "Example: Model")   -- application name
