...
module Rort.Examples.Gltf where
...
main = do
  ...
  withWindow width height "Example: Gltf" $ \win -> do
    ...
    runResourceT $ do
      let
        cfg = VkSettings { requiredExtensions =
                         ...
                         , applicationInfo =
                             Vk.ApplicationInfo
                               (Just "Example: Gltf")   -- application name
      ...
      tex <-
        texture r $ do
          t <- liftIO $ loadTexture "data/viking_room.png"
          (imgData :: [Word8], (w, h)) <-
          ...
          submit ctx r $ \swapchain -> do
            ...
            let
              ...
              passes = [ DrawRenderPass
                           ...
                           , drawSubpasses = [ DrawSubpass draws ]
                           }
                       ]
            pure passes
